...
-- |"conditional" priority is either an if-then-else expression or "arithmetic"
--  priority
pCondExpr :: TBNParser Expr
pCondExpr = "conditional expression" <@>
      origConstr3 ExprCondition $%
        (,,) <$ consume TokIf <*> pExpr
...
pBinaryArithExpr = do
    exp1 <- ("binary arithmetic expression" <@>
             ...
             <|> pOnionExpr)
    option exp1 (origConstr2 ExprSequence $% (,) <$> return exp1 <* consume TokSemi <*> pExpr)
