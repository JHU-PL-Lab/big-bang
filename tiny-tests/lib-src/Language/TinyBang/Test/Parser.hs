...
import Data.ExtensibleVariant (inj)
...
tests = TestLabel "Miscellaneous parser tests" $ TestList
  ...
  , xPars "1 & 2 &- int & ()" $
          multiOnion [ inj $ A.OnionSub (multiOnion [pi 1, pi 2]) projInt
                     , inj $ A.PrimUnit]
  , xPars "1 & 2 &. int &- int" $ inj $
          A.OnionSub
            (inj $ A.OnionProj (multiOnion [pi 1, pi 2]) projInt)
            projInt
  , xPars "1 == 2 &- int" $ inj $
          A.OnionSub (inj $ A.EagerOp A.Equal (pi 1) (pi 2)) projInt
  , xPars "1 + 2 == 3" $ inj $
          A.EagerOp A.Equal
            (inj $ A.LazyOp A.Plus (pi 1) (pi 2))
            (pi 3)
  , xPars "1 + 2 == 2 + 1" $ inj $
          A.EagerOp A.Equal
            (inj $ A.LazyOp A.Plus (pi 1) (pi 2))
            (inj $ A.LazyOp A.Plus (pi 2) (pi 1))
...
--          multiAppl [varY, simpleScape "x" varX]
  , xPars "`A `B ()" $
          simpleLabel "A" $ simpleLabel "B" $ inj A.PrimUnit
  , xPars "`A (x -> x)" $
  ...
  , xPars (tbCase "x"
           ...
           , "fun -> (x -> x)"])
          (multiAppl
           [ multiOnion $ reverse $ map inj
             [ A.Scape (A.Pattern id_ patInt) $ pi 5
             , A.Scape (A.Pattern id_ patChar) $ inj $ A.PrimChar 'a'
             , A.Scape (A.Pattern id_ patUnit) $ inj A.PrimUnit
