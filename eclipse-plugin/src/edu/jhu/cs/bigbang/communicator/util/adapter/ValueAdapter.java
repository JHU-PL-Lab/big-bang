...
public class ValueAdapter implements JsonDeserializer<Value> {
	...
	public Value deserialize(JsonElement je, Type typeOfSrc,
		...
		if (type == "VInt"){
			...
			onionOpGb.registerTypeHierarchyAdapter(OnionOp.class, new OnionOpAdapter());
			Gson onionOpG = onionOpGb.create();
			
			OnionOp onionOp = onionOpG.fromJson(jo.get("onionOp").getAsJsonObject(), OnionOp.class);
			
			GsonBuilder anyProjectorGb = new GsonBuilder();
			anyProjectorGb.registerTypeHierarchyAdapter(AnyProjector.class, new AnyProjectorAdapter());
			Gson anyProjectorG = anyProjectorGb.create();
			
			AnyProjector anyProjector = anyProjectorG.fromJson(jo.get("anyProjector").getAsJsonObject(), AnyProjector.class);
			...
			patternGb.registerTypeHierarchyAdapter(Pattern.class, new PatternAdapter());
			...
			Pattern pattern = patternG.fromJson(jo.get("pattern").getAsJsonObject(), Pattern.class);
			Expr expr = exprG.fromJson(jo.get("expr").getAsJsonObject(), Expr.class);
