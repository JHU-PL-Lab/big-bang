...
compatibility x pat visits = do
  _debug $ "Checking compatibility for " ++ display x ++ " with "
              ++ display pat
  result <-
    case pat of
      ...
      PDisj _ pat' pat'' -> do
        ...
        case mb2 of
          Just _ -> return mb2
          Nothing -> compatibility x pat' visits
      PSubst _ x' (AstList _ pats) -> do
        ...
        case pattern of
          Just (VPattern _ (AstList _ ys) pat')
            ...
            | otherwise ->
                let pat'' = patternSubstitute
                              (Map.fromList $ zip ys pats) pat' in
                compatibility x pat'' $ Set.insert pat' visits
          Just _ ->
            error "Projection produced non-pattern for pattern projector"
          Nothing -> return Nothing
      PRec _ y pat' ->
          ...
          patternFromScape v = case v of
            VScape _ xp _ -> do
              ...
              case v' of
                ...
                Just _ -> error $
                  "Pattern projected produced non-pattern value " ++ display v'
                Nothing -> return Nothing
            _ -> error $ "Scape projection produced non-scape value "
                            ++ display v
      PVar _ y -> Just . Map.singleton y <$> flowLookup x
      PNone _ -> return Nothing
  v <- flowLookup x
  _debug $ "Compatibility result for " ++ display x ++ " (having value "
              ++ display v ++ ") with " ++ display pat ++ " is: "
              ++ display result
  return result
  where
